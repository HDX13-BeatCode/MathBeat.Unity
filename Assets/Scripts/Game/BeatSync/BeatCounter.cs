	...
	public class BeatCounter : MonoBehaviour {
		...
		void Awake ()
		{
			...
			if (beatOffset != BeatValue.None) {
				sampleOffset = (60f / (audioBpm * BeatDecimalValues.values[(int)beatOffset]) * beatOffsetValue)
		...
		/// <summary>
		/// Initializes and starts the coroutine that checks for beat occurrences.
		/// The nextBeatSample field is initialized to exactly match up with the sample
		/// that corresponds to the time the audioSource clip started playing (via PlayScheduled).
		/// </summary>
