...
namespace MathBeat.GUI
{
	public class List : MonoBehaviour 
	{
		...
		public delegate void OnItemLoadedHandler (BaseAdapter item);

		public OnItemLoadedHandler onItemLoaded;

		public void ItemLoaded(BaseAdapter item, bool clear = false)
		{
		...
		public delegate void OnItemSelectedHandler (BaseAdapter item);

		public OnItemSelectedHandler onItemSelected;

		public void ItemSelected(BaseAdapter item, bool clear = false)
		{
		...
		private List<BaseAdapter> _itemsList;
		...
		public void Create(int items, BaseAdapter listItemPrefab)
		{
			...
			_itemsList = new List<BaseAdapter> ();

			for (int i = 0; i < itemsToInstantiate; i++)
			{
				BaseAdapter item = CreateNewItem (listItemPrefab, i, _itemSize);
		...
		private BaseAdapter CreateNewItem(BaseAdapter prefab, int index, float dimension)
		{
			...
			return instance.GetComponent <BaseAdapter> ();
		}


		void HandleOnSelectedHandler (BaseAdapter item)
		{
			ItemSelected (item);
		}
		...
		{
			BaseAdapter firstItem = _itemsList [0];
			BaseAdapter lastItem = _itemsList [_itemsList.Count - 1];
